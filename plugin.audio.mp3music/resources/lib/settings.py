...
DATA_PATH = os.path.join(xbmcvfs.translatePath('special://profile/addon_data'
                                            '/plugin.audio.mp3music'), '')


def cookie_jar():
    return commontasks.create_file(DATA_PATH, "cookiejar.lwp")
...
def artist_icons():
    return commontasks.create_directory(DATA_PATH, "artist_icons")
...
def favourites_file_artist():
    return commontasks.create_file(DATA_PATH, "favourites_artist.list")


def favourites_file_album():
    return commontasks.create_file(DATA_PATH, "favourites_album.list")


def id3tags_list():
    return commontasks.create_file(DATA_PATH, "id3tags.list")


def download_list():
    return commontasks.create_file(DATA_PATH, "downloads.list")


def favourites_file_songs():
    return commontasks.create_file(DATA_PATH, "favourites_songs.list")
...
def music_dir():
    if __addon__.getSetting('music_dir') == "set":
        return commontasks.create_directory(DATA_PATH, "music")
    else:
