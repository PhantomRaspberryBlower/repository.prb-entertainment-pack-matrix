...
_clear_downloads = __language__(30046)
_start_downloads = __language__(30047)
_downloads = __language__(30005)
_playlist = __language__(30019)
_cleared = __language__(30020)
_mp3_music = __language__(30023)
_browse_songs_or_play_album = __language__(30025)
_browse = __language__(30024)
_play_now = __language__(30026)
_creating_your_playlist = __language__(30027)
_select_group = __language__(30028)
_added_to_favourites = __language__(30029)
_removed_from_favourites = __language__(30030)
_remove = __language__(30048)
_artists = __language__(30031)
_top_albums =  __language__(30032)
_new_albums =  __language__(30033)
_compilations =  __language__(30034)
_search_lists = __language__(30049)
_search_artists = __language__(30035)
_search_albums = __language__(30036)
_search_songs = __language__(30037)
_favourite_lists = __language__(30050)
_favourite_artists = __language__(30038)
_favourite_albums = __language__(30039)
_favourite_songs = __language__(30040)
_instant_mix_favourite_albums = __language__(30041)
_instant_mix_favourite_songs = __language__(30042)
_clear_playlist = __language__(30043)
_settings = __language__(30044)
_instant = __language__(30075)
_clear = __language__(30076)
_top = __language__(30077)
__artists = __language__(30078)
_new = __language__(30079)
__albums = __language__(30080)
_best_compilations = __language__(30081)
_new_compilations = __language__(30082)
_major_hits = __language__(30083)
_nightclub_hits = __language__(30084)
_chillout_hits = __language__(30085)
_tributes_and_covers = __language__(30086)
_events = __language__(30087)
_err_downloading = __language__(30088)
_err_getting_webpage = __language__(30089)
_clear_download_list = __language__(30046)
_are_you_sure = __language__(30052)
_adding_to_your_playlist = __language__(30053)
_mp3_music_downloads_complete = __language__(30054)
_downloading = __language__(30055)
_n_of_n_tracks_downloaded = __language__(30056)
_all_songs = __language__(30057)
_all_albums = __language__(30058)
_add_new_group = __language__(30059)
_create_new_group = __language__(30060)
_artist_info = __language__(30061)
_unable_to_download_artist_info = __language__(30062)


_artist_info_heading = __language__(30063)
_clear_download_lock_heading = __language__(30064)
_added_to_favourite_artists_heading = __language__(30065)
_remove_from_favourite_artists_heading = __language__(30066)
_download_album_heading = __language__(30067)
_play_browse_album_heading = __language__(30068)
_queue_album_heading = __language__(30069)
_add_to_favourite_albums_heading = __language__(30070)
_remove_from_favourite_albums_heading =  __language__(30071)
_remove_heading = __language__(30072)
_download_song_heading = __language__(30073)
_queue_song_heading = __language__(30074)
_play_song_heading = __language__(30091)
_add_to_favourite_songs_heading = __language__(30092)
_remove_from_favourite_songs_heading = __language__(30093)
...
artfolder = xbmcvfs.translatePath(os.path.join(resources, 'media/'))
artgenre = xbmcvfs.translatePath(os.path.join(artfolder, 'genre/'))
...
#    addDir(_artists, "url", 21, artfolder + 'artists.jpg', '', '', '')
    ...
    if os.path.isfile(__download_list__):
        ...
        for list in search_list:
            if list != '':
                ...
                if len(trackname) < 1:
                    title = ('%s - %s (%s)' % (artist, album, year)).replace('&amp;', 'and')
                    addDir(title, url, 5, niconimage, 'downloadlist', year, genre)
                else:
                    title = ('%s - %s (%s)' % (artist, trackname, year)).replace('&amp;', 'and')
                    addDirAudio(title,
                                url,
                                10,
                                niconimage,
...
def all_artists(name, url):
    ...
    addDir('>> Next page', nxtpgurl, 31,
           xbmcvfs.translatePath(os.path.join('special://home/addons/'
...
def compilations_list(name, url, iconimage, page):
    ...
    for url, d1, iconimage, cl, url2, title, cl, artist, year, prc in match:
        ...
        addDir('Various Artists - ' + title.replace('&amp;', 'and'), url, 5, iconimage, 'albums', year, '')
    addDir('>> Next page', nxtpgurl, 401, xbmcvfs.translatePath(os.path.join(
...
def search_songs(query):
    ...
    url = (MUSIC_DIR_URL + '/search.html?text=%s&all=songs' %
           urllib.parse.quote_plus(query.replace(' - ', ' ').
                             replace('-', ' ').replace(' FT ', ' ')))
    link = downloads.get_url(url)
    ...
    for id, songurl, song, d1, artisturl, artist, d2, albumurl, album in match:
        ...
        liz.setArt({'fanart': fetch_fanart(artist), 'thumb': iconimage})
...
def play_album(name, url, iconimage, mix, clear, genre):
    ...
    if filtered_url == 'Failed to parse URL! :(':
        return filtered_url
...
def clear_playlist():
    pl = xbmc.PlayList(xbmc.PLAYLIST_MUSIC)
    pl.clear()
    commontasks.notification(_playlist, _cleared, iconart, 2000)
...
def addDir(name, url, mode, iconimage, type, year, genre):
    ...
    u = (sys.argv[0] +
         ...
         "&genre=" + urllib.parse.quote_plus(genre))
    ok = True
    contextMenuItems = []
    if type == "artists":
    ...
    if 'album' in type:
        ...
        download_album = ('%s?name=%s&url=%s&iconimage=%s&year=%s&genre=%s&mode=202' %
                          (sys.argv[0],
                           ...
                           iconimage,
                           year,
                           urllib.parse.quote(genre)))
        contextMenuItems.append((_download_album_heading,
                                 'RunPlugin(%s)' % download_album))
        if __queue_albums__:
    ...
    liz.setArt({'fanart': fetch_fanart(name), 'thumb': iconimage})
...
def addDirAudio(name, url, mode, iconimage, songname, artist, album, dur, type, year, genre):
    ...
    u = sys.argv[0] + ("?url=" + urllib.parse.quote_plus(url) +
                       ...
                       "&name=" + urllib.parse.quote_plus(name) +
                       "&iconimage=" + urllib.parse.quote_plus(iconimage) +
                       "&songname=" + urllib.parse.quote_plus(songname) +
    ...
    if not __hide_fanart__:
        if len(str(artist)) > 0:
            liz.setArt({'fanart': get_current_artist_images(artist)[1]})
        else:
            liz.setArt({'fanart': fetch_fanart(name)})
    ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),
                                     url=u, listitem=liz)
    return ok
...
def show_artist_info(artistname):
    """
...
def show_artist_info_old(artist):
    '''
    ...
    try:
        if len(artist) > 0:
            ...
            window = xbmcgui.WindowXMLDialog('artist-info.xml',
                                             __addon__.getAddonInfo('path'))
...
for item in param_names:
    try:
        if param_names.index(item) > 7:
            ...
            locals()[item] = urllib.parse.unquote_plus(params[item])
    except:
        pass
