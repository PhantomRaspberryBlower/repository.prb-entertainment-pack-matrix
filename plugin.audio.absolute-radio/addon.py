...
__cookie_file__ = xbmcvfs.translatePath(os.path.join('special://profile/addon_data/'
...
def login_absolute_radio(username, password):
    ...
    """
    cookie_jar = http.cookiejar.LWPCookieJar(__cookie_file__)
    try:
        cookie_jar.load()
    except:
        pass
        ...
        if _hide_artist_artwork != 'true':
            ...
            if my_title != previous_title:
                ...
                window.setProperty('ArtistFanart',
                                   artist_image)
                window.setProperty('ArtistThumb',
                                   icon)
