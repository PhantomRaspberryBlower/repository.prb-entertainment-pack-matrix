...
# Dreamcatcher, Exodus, PressPlay, MP3Music, The Oath, Youtube DL
    ...
    if os.path.isfile(__addons_download_paths_list__):
        ...
        for list in addons_list:
            if list != '':
                ...
                addon_data_dir = xbmcvfs.translatePath(os.path.join('special://profile/addon_data',
                	                                                split_list[1]))
...
def _create_settings_xml(addon=None):
    ...
    if os.path.isfile(os.path.join(resources, 'settings.xml')):
        ...
        addon_data_dir = xbmcvfs.translatePath(os.path.join('special://profile/addon_data', addon))
